...
type
TXPManifestResourceDetails = class (TResourceDetails)
public
  class function GetBaseType : string; override;
  procedure InitNew; override;
  procedure GetText (xml : TStrings);
  procedure SetText (xml : TStrings);
end;
...
procedure TXPManifestResourceDetails.GetText(xml: TStrings);
begin
  data.Seek(0, soFromBeginning);
  xml.LoadFromStream(data);
end;
...
procedure TXPManifestResourceDetails.SetText(xml: TStrings);
begin
  data.Clear;
  xml.SaveToStream(data);
end;
